...
def visualize_recording(
    ...
    # Load bimanual transform to convert right-arm-base positions to left-arm-base frame.
    # Right arm action is stored in right-arm base frame; left arm is already in left-arm base.
    T_left_from_right: np.ndarray = np.eye(4, dtype=np.float64)
    calib_path = rec_dir / "calibration_results.json"
    if calib_path.exists():
        with open(calib_path) as _cf:
            _calib = json.load(_cf)
        _mat = _calib.get("bimanual_transform", {}).get("right_base_to_left_base")
        if _mat is not None:
            T_left_from_right = np.linalg.inv(np.array(_mat, dtype=np.float64))

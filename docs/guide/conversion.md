...
ZED stereo cameras support two depth estimation backends, selected with
`--stereo-method`:

| Backend | Flag | Description |
|---|---|---|
| ZED SDK (default) | `--stereo-method zed` | On-device NEURAL_LIGHT depth from the ZED SDK. Fast, no extra GPU setup required. |
| Fast Foundation Stereo | `--stereo-method ffs` | [Fast Foundation Stereo](https://github.com/NVlabs/Fast-FoundationStereo) - a foundation model for stereo depth. Produces higher-quality depth, especially at object boundaries and on thin structures. Requires a CUDA GPU; see [Installation](installation.md#fast-foundation-stereo-optional) for setup. |
...
!!! warning "Fast Foundation Stereo is experimental"
    Fast Foundation Stereo support in `rd convert` is experimental. It is a
    GPU-heavy model and may be slow depending on your hardware.
